...
const validate_event = ({
  ...
  if (!draft) {
    if (!des.length || des.length > 400) {
      ...
      throw new Error(
        "@@[Thrown] The description provided must be under 200 characters"
      );
    }
    ...
    if (!banner.length) {
      ...
      throw new Error(
        "@@[Thrown] You must provide a banner to publish your post"
      );
    }
    ...
    if (!content.blocks.length) {
      ...
      throw new Error(
        "@@[Thrown] You must provide some content to publish your post"
      );
    }
